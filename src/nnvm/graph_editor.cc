...
/*
 * This creates a subgraph represented by the output entries and keeps
 * the subgraph in a node. In the meanwhile, we change the original node entries
...
nnvm::NodePtr CutCreateSubgraphNode(const std::vector<nnvm::NodeEntry *> &output_entries,
  ...
  for (size_t i = 0; i < output_entries.size(); i++)
    *output_entries[i] = nnvm::NodeEntry{n, i, 0};
