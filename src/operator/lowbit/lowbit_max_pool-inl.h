 ...
 * \file lowbit_max_pool-inl.h
 * \brief
 * \author Ziheng Jiang
*/

#ifndef MXNET_OPERATOR_LOWBIT_MAX_POOL_INL_H_
#define MXNET_OPERATOR_LOWBIT_MAX_POOL_INL_H_
...
struct LowbitMaxPoolParam : public dmlc::Parameter<LowbitMaxPoolParam> {
  TShape kernel;
  TShape stride;
  TShape pad;
  int convention;
  DMLC_DECLARE_PARAMETER(LowbitMaxPoolParam) {
...
Operator* CreateOp(LowbitMaxPoolParam param, int dtype);

class LowbitMaxPoolProp : public OperatorProperty {
 public:
  ...
  OperatorProperty* Copy() const override {
    LowbitMaxPoolProp *prop_sym = new LowbitMaxPoolProp();
    prop_sym->param_ = this->param_;
    return prop_sym;
  }

  std::string TypeString() const override {
    return "lowbit_max_pool";
  }
