 ...
 * \file lowbit_lrn-inl.h
 * \brief
 * \author Ziheng Jiang
*/
#ifndef MXNET_OPERATOR_LOWBIT_LRN_INL_H_
#define MXNET_OPERATOR_LOWBIT_LRN_INL_H_
...
struct LowbitLRNParam : public dmlc::Parameter<LowbitLRNParam> {
  float alpha;
  float beta;
  float knorm;
  uint32_t nsize;
  DMLC_DECLARE_PARAMETER(LowbitLRNParam) {
...
};  // struct LowbitLRNParam

template<typename xpu>
Operator *CreateOp(LowbitLRNParam param, int dtype);

class LowbitLRNProp : public OperatorProperty {
 public:
...
#endif  // MXNET_OPERATOR_CONTRIB_lowbit_LRN_INL_H_
