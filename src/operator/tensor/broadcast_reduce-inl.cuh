...
void Reduce(Stream<gpu> *s, const TBlob& small, const OpReqType req,
  ...
  if (req == kNullOp) return;
  cudaStream_t stream = Stream<gpu>::GetStream(s);
  ReduceImplConfig<ndim> config = ConfigureReduceImpl<ndim, DType>(small.shape_, big.shape_, NULL, NULL);
...
void Reduce(Stream<gpu> *s, const TBlob& small, const OpReqType req,
  ...
  if (req == kNullOp) return;
  cudaStream_t stream = Stream<gpu>::GetStream(s);
  ReduceImplConfig<ndim> config = ConfigureReduceImpl<ndim, DType>(small.shape_, big.shape_, &lhs.shape_, &rhs.shape_);
